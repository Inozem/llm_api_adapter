...
@pytest.mark.unit
def test_known_deepseek_organization_is_distinct_from_unknown_and_loads_lazily(
    monkeypatch,
    isolated_plugin_runtime,
):
    for module_name in (
        "llm_api_adapter_deepseek.plugin",
    ...
    with pytest.raises(ValueError, match="Unsupported organization: deepseek-like"):
        UniversalLLMAPIAdapter(
            organization="deepseek-like",
    ...
    assert str(raised.value) == (
        "Organization 'deepseek' is not installed. "
        "Install it with: pip install llm-api-adapter-deepseek"
    )
    assert "llm_api_adapter_deepseek" not in sys.modules

    entry_point = FakeEntryPoint(
        name="deepseek",
        value="test_plugins.deepseek:PLUGIN",
        plugin=_test_plugin(
            organization="deepseek",
            model_metadata=_organization_model_metadata("deepseek"),
        ),
    )
    installed_entry_points.append(entry_point)
    ...
    assert isinstance(adapter.adapter, PluginTestAdapter)
    assert adapter.adapter.company == adapter.adapter.service_provider == "deepseek"
    assert adapter.adapter.model_spec is not None
    assert resolve_model_spec(
        isolated_plugin_runtime[2],
        "deepseek",
