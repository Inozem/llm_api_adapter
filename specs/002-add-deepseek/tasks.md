...
- [X] T024 [US3] Implement Responses input/output mapping for application function tools, normal tool results, tool choice (forcing `reasoning_level="none"` with a warning for named selection and tool-result continuation), portable JSON Schema, response-model validation, user image URL/data parts, and adapter-local capability preflight that rejects provider-built-in tools or explicit parallel control before transport in `packages/organizations/deepseek/src/llm_api_adapter_deepseek/adapter.py`.
...
- [X] T030 [P] [US4] Write the installed-distribution non-live file boundary test proving that document input cannot start a DeepSeek request in `packages/organizations/deepseek/tests/e2e/test_file_contract.py`.
...
- [X] T034 [US4] Run and make green the document, usage, cost, and installed-file-boundary checks in `packages/organizations/deepseek/tests/test_deepseek_adapter.py` and `packages/organizations/deepseek/tests/e2e/test_file_contract.py` without setting a DeepSeek credential.
...
- [X] T040 Add DeepSeek change filters and workflow dispatch to `.github/workflows/ci-dev.yml` and `.github/workflows/ci-main.yml`, then add TestPyPI publication, exact candidate installation, `DEEPSEEK_API_KEY` validation, plugin discovery, and the full applicable `e2e_deepseek` post-publish job to `.github/workflows/ci-dev-release.yml`.
- [X] T041 Update the contributor and release guidance for independently versioned DeepSeek artifacts and the credential boundary in `CONTRIBUTING.md` and `specs/002-add-deepseek/quickstart.md`.
- [X] T042 Update the living baseline contract with the DeepSeek optional package, its direct-document rejection boundary, and opaque continuation metadata that is never rendered or logged in `specs/001-baseline-contract/spec.md`.
...
- [X] T046 Before promoting the exact reviewed candidate through a staging pull request to protected `dev`, verify only that `DEEPSEEK_API_KEY` is present in the maintainer environment (never display its value), then provide `python -m pytest -v --import-mode=importlib -m e2e_deepseek --rootdir=. tests/e2e packages/organizations/deepseek/tests/e2e` for maintainer invocation. The suite reads the key from the environment and runs every applicable shared and package-local DeepSeek E2E contract; record only sanitized results after the maintainer runs it. Promote only after it passes; the automatic `dev` TestPyPI E2E lane remains the independent post-publish gate.
  - Superseded smoke evidence (2026-09-17): the package-local live contract passed (`1 passed in 1.85s`); it did not satisfy the expanded full-suite gate.
  - Sanitized full-suite evidence (2026-09-17): the maintainer-run full marker suite had no failed selected test; its only initially skipped async structured-output subtest passed after the portable Core schema fixture was corrected (`1 passed in 1.31s`). Together, all 17 selected DeepSeek E2E contracts passed. No credential or provider payload was recorded.
